// components/recuadros/subFamiliasProductosPicker.tsx
...
export function SubfamiliasProductosPicker({
  disabled,
  familias,
  subfamilias,
  selected,
  ...
  useEffect(() => {
    if (familiaId == null && familias.length) {
      setFamiliaId(familias[0].IdFamiliaProducto);
    }
  }, [familias, familiaId]);
  ...
  const subsFiltradas = useMemo(() => {
    ...
    return subfamilias
      .filter((s) => s.IdFamiliaProducto === familiaId)
      .map((s) => ({
        id: s.IdSubFamiliaProducto,
  ...
  const selectedItems = useMemo(() => {
    ...
    for (const s of subfamilias) {
      map.set(s.IdSubFamiliaProducto, s.Descripcion);
    }
      ...
      {isView ? (
        ...
        <>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-3">
            ...
            <div className="text-xs text-slate-400 mt-2">
              Selecciona y pulsa Add para asignar al producto.
            </div>
          </div>
