...
import { previewMensual, generarMensual, type PreviewMensual } from "~/lib/facturacionMensualRest";

export default function FacturacionMensualModal({ onClose, onGenerated }: { onClose: () => void; onGenerated?: () => void }) {
  const { t } = useTranslation(["escola", "common"]);
  const MESES = ["", t("escola:facturacion.meses.1"), t("escola:facturacion.meses.2"), t("escola:facturacion.meses.3"), t("escola:facturacion.meses.4"), t("escola:facturacion.meses.5"), t("escola:facturacion.meses.6"), t("escola:facturacion.meses.7"), t("escola:facturacion.meses.8"), t("escola:facturacion.meses.9"), t("escola:facturacion.meses.10"), t("escola:facturacion.meses.11"), t("escola:facturacion.meses.12")];
  const hoy = new Date();
  ...
  // Exclusiones y descuentos editables
  ...
  const cargarPreview = async () => {
    ...
    setResultado(null);
    try {
      const p = await previewMensual(mes, anyo);
      setPreview(p);
      // Excluir por defecto los ya facturados
  ...
  // Recalcular totales en vivo según exclusiones y descuento ajustado
  ...
  const generar = async () => {
    if (!preview) return;
    if (!confirm(t("escola:facturacion.confirmGenerarMensual", { nFacturas, total: totalGlobal.toFixed(2) }))) return;
    setLoading(true);
    setError(null);
    try {
      const r = await generarMensual({
        mes,
        anyo,
      ...
      setError(e?.response?.data?.error ?? t("escola:facturacion.errorGenerandoFacturas"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden max-h-[90vh] flex flex-col">
          <div className="px-6 py-4 border-b border-slate-200">
            <h2 className="text-lg font-bold text-slate-900">{t("escola:facturacion.tituloMensual")}</h2>
            <p className="text-sm text-slate-500">{t("escola:facturacion.subtituloMensual")}</p>
          </div>
          ...
          <div className="px-6 py-4 space-y-4 overflow-y-auto flex-1">
            ...
            {resultado && (
              <div className="p-4 rounded-lg bg-green-50 text-green-800 text-sm border border-green-200">
                <b>{resultado.creadas}</b> {t("escola:facturacion.facturasGeneradasCorrectamente")}
            ...
            <div className="flex flex-wrap items-end gap-3">
              <div>
                <label className="text-sm font-medium text-slate-700">{t("escola:facturacion.mes")}</label>
              ...
              <div>
                <label className="text-sm font-medium text-slate-700">{t("escola:facturacion.anyo")}</label>
            ...
            {preview && (
              <div className="space-y-3">
                {preview.facturas.length === 0 && <p className="text-sm text-slate-500">{t("escola:facturacion.sinAlumnosActivos")}</p>}
                    ...
                    <div key={f.idCliente} className={`rounded-lg border px-3 py-2 ${excluido ? "border-slate-200 bg-slate-50 opacity-60" : "border-slate-200"}`}>
                      <div className="flex items-center justify-between gap-2">
                        <label className="flex items-center gap-2 font-medium text-slate-900 text-sm">
                          ...
                          {f.nombreAlumno}
                          {f.yaFacturado && <span className="text-xs px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">{t("escola:facturacion.yaFacturadoMes")}</span>}
                        </label>
                ...
                {preview.facturas.length > 0 && (
                  <div className="rounded-lg bg-slate-900 text-white px-4 py-3 flex items-center justify-between">
                    <span className="text-sm">{t("escola:facturacion.seGeneraran")} <b>{nFacturas}</b> {t("escola:facturacion.facturas")}</span>
